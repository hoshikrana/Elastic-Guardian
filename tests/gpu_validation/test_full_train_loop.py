...
class TestFullTrainLoop(unittest.TestCase):
    ...
    def test_kernel_train_step(self):
        ...
        kernel = TrainingKernel(
            ...
            optimizer_type="sgd",
            learning_rate=0.01,
            loss_fn=self._sum_loss,
        )
        # Use a simple forward that returns a scalar sum for .backward()
        original_forward = model.forward
        model.forward = lambda **kwargs: original_forward(
            kwargs.get("input", torch.randn(2, 16))
        )
    ...
    def test_kernel_multiple_steps(self):
        ...
        model = nn.Linear(16, 4)
        model.forward = lambda **kwargs: model.__class__.forward(
            model, kwargs.get("input", torch.randn(2, 16))
        )
        kernel = TrainingKernel(
            model=model,
    ...
    def test_kernel_with_callbacks(self):
        ...
        model = nn.Linear(16, 4)
        model.forward = lambda **kwargs: nn.Linear.forward(
            model, kwargs.get("input", torch.randn(2, 16))
        )
        kernel = TrainingKernel(
            model=model,
