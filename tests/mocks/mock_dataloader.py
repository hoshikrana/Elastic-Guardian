...
class MockBatchIterator:
    ...
    def __init__(self, batch_size: int = 4, seq_len: int = 32, vocab_size: int = 1000, num_batches: int = 5):
...
class MockDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, size: int = 100, seq_len: int = 32):
        self.data = [
            {"input_ids": torch.randint(0, 1000, (seq_len,)), "attention_mask": torch.ones(seq_len, dtype=torch.long)}
            for _ in range(size)
        ]
