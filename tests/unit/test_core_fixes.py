...
class TestImprovedAnalyticalEstimator:
    ...
    def test_lora_uses_less_memory_than_full_ft(
        ...
        report_ft = estimator.estimate(
            topology_single_gpu, llama_7b_profile, plan_full_ft
        )
        report_lora = estimator.estimate(
            topology_single_gpu, llama_7b_profile, plan_lora
        )
    ...
    def test_gradient_checkpointing_reduces_activations(
        ...
        report_no_ckpt = estimator.estimate(
            topology_single_gpu, llama_7b_profile, plan_no_ckpt
        )
        report_with_ckpt = estimator.estimate(
            topology_single_gpu, llama_7b_profile, plan_with_ckpt
        )

        # Checkpointing should reduce activations by ~80%
        assert (
            report_with_ckpt.activations_bytes < report_no_ckpt.activations_bytes * 0.3
        )
    ...
    def test_memory_estimate_structure(
        ...
        report = estimator.estimate(
            topology_single_gpu, llama_7b_profile, training_plan_lora
        )
