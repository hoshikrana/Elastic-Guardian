...
class TrainingKernel:
    ...
    def train_step(self, batch: Dict[str, Any], step: int) -> float:
        ...
        try:
            ...
            # Checkpointing
            if self.checkpoint_mgr and self.checkpoint_mgr.should_save(step, loss.item()):
                self.checkpoint_mgr.checkpoint(step, loss.item(), self.model.state_dict())
            ...
            raise EGXError(str(e), recoverable=True, suggested_action=RecoveryAction.RETRY) from e
