...
class ModelProfile:
    ...
    def __init__(self, name: str, params: int, hidden_dim: int, layers: int, arch: str):
        self.name = name
        self.params = params
...
class ModelIntrospector:
    ...
    def introspect(self, model: Any, name: str = "unknown") -> ModelProfile:
        ...
        if hasattr(model, "config"):
            config = model.config
            hidden_dim = getattr(config, "hidden_size", 0) or getattr(
                config, "d_model", 0
            )
            layers = getattr(config, "num_hidden_layers", 0) or getattr(
                config, "n_layer", 0
            )
        ...
        return ModelProfile(
            name=name, params=params, hidden_dim=hidden_dim, layers=layers, arch=arch
        )
