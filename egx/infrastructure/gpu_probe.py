...
class GPUProber:
    ...
    def probe(self) -> List[GPUSpec]:
        ...
        if not gpus:
            logger.warning("No compatible GPUs detected. Falling back to CPU-only mode (Law 9: Explicit).")
            gpus = [self._cpu_spec()]
    ...
    def _get_peer_ids(self, handle: Any, count: int) -> Tuple[int, ...]:
        ...
        for j in range(count):
            try:
                if pynvml.nvmlDeviceGetP2PStatus(handle, j, pynvml.NVML_P2P_CAPS_INDEX_READ) == pynvml.NVML_P2P_STATUS_OK:
                    peers.append(j)
            except: pass
        return tuple(peers)