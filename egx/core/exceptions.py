...
class GPUNotFoundError(HardwareError):
    def __init__(
        self,
        message: str = "No CUDA/MPS GPUs detected",
        context: Optional[ErrorContext] = None,
    ):
        super().__init__(
            message,
...
class ThermalThrottleError(HardwareError):
    def __init__(
        self, gpu_id: int, temp_c: float, context: Optional[ErrorContext] = None
    ):
        super().__init__(
            f"GPU {gpu_id} Throttling: {temp_c}°C",
            recoverable=True,
            suggested_action=RecoveryAction.ABORT,
            context=context,
        )
...
class BoolAsIntError(MemoryValidationError):
    def __init__(self, field: str, value: bool):
        super().__init__(
            f"Type Trap: Field '{field}' received bool ({value}) instead of int. Law 10 violation."
        )


class OutOfMemoryError(EGXMemoryError):
    def __init__(
        self, msg: str = "VRAM Out of Memory", context: Optional[ErrorContext] = None
    ):
        super().__init__(
            msg,
            recoverable=True,
...
class InsufficientHardwareError(PlanningError):
    def __init__(self, recs: list[str], context: Optional[ErrorContext] = None):
        super().__init__(
            "Hardware insufficient for model. Recommendations: " + ", ".join(recs),
            recoverable=False,
            suggested_action=RecoveryAction.ABORT,
            context=context,
        )
...
class NaNLossError(TrainingError):
    def __init__(self, step: int, context: Optional[ErrorContext] = None):
        super().__init__(
            f"NaN Loss detected at step {step}",
            recoverable=True,
...
class InfGradientError(TrainingError):
    def __init__(self, step: int, layer: str, context: Optional[ErrorContext] = None):
        super().__init__(
            f"Inf Gradient in layer '{layer}' at step {step}",
            recoverable=True,
            suggested_action=RecoveryAction.RETRY,
            context=context,
        )
...
class CheckpointCorruptError(TrainingError):
    def __init__(self, path: str, context: Optional[ErrorContext] = None):
        super().__init__(
            f"Checkpoint corrupted (SHA256 mismatch): {path}",
            recoverable=True,
            suggested_action=RecoveryAction.RELOAD_CHECKPOINT,
            context=context,
        )
...
class DeadlockError(TrainingError):
    def __init__(
        self, timeout_s: float, last_step: int, context: Optional[ErrorContext] = None
    ):
        super().__init__(
            f"Training Deadlock (Heartbeat > {timeout_s}s). Last Step: {last_step}",
            recoverable=True,
...
class CircularDependencyError(PluginError):
    def __init__(self, cycle: list[str], context: Optional[ErrorContext] = None):
        super().__init__(
            f"Circular Dependency in modules: {' -> '.join(cycle)}",
            recoverable=False,
            suggested_action=RecoveryAction.ABORT,
            context=context,
        )
