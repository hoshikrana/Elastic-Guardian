...
def train(
    model: Optional[str] = None, scratch: bool = False, config: Optional[str] = None
):
    """Starts an automated training session."""
    target = model or config or "unnamed_model"
    mode_str = "SCRATCH" if scratch else "FINETUNE"
    console.print(
        f"[bold cyan]EGX Trainer: Starting {mode_str} session for '{target}'...[/]"
    )
...
def probe():
    ...
    try:
        ...
        if _HAS_RICH:
            ...
            for gpu in topo.gpus:
                print(
                    f"GPU {gpu.device_id}: {gpu.name} ({gpu.vram_bytes // (1024**3)} GB)"
                )
