...
def run_showcase():
    ...
    console.print(
        "✔ Analysis Complete! Selected Strategy: [bold green]QLoRA / 4-bit Quantization[/]"
    )
    ...
    res_table.add_row(
        "Status:",
        "[bold green]Success[/]" if result.get("success") else "[bold red]Failed[/]",
    )
    res_table.add_row("Final Loss:", f"[bold cyan]{result.get('final_loss', 'N/A')}[/]")
    res_table.add_row(
        "Selected Mode:", f"[bold magenta]{result.get('mode', 'UNKNOWN')}[/]"
    )
