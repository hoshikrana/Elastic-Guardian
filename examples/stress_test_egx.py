...
def run_stress_test():
    ...
    for cfg in configs:
        ...
        if est_memory_gb > current_ram_gb:
            console.print(
                f"[bold red]❌ Memory Check Failed![/] Estimated need: {est_memory_gb:.2f} GB. Available: {current_ram_gb:.2f} GB."
            )
            ...
            if res.get("success"):
                ...
                results.append(
                    (
                        name,
                        f"{params_mil:.1f}M",
                        f"[bold green]Success ({res.get('mode')})[/]",
                    )
                )
                ...
                results.append(
                    (name, f"{params_mil:.1f}M", "[bold red]Framework Failed[/]")
                )
                break
